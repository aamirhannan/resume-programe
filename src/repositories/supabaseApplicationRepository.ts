...
export class SupabaseApplicationRepository implements IApplicationRepository {
    ...
    async findByApplicationID(applicationID: string): Promise<IApplication | null> {
        ...
        if (error) {
           if (error.code !== 'PGRST116') console.error('Error finding app by ID:', error);
           return null;
        }
    ...
    async update(applicationID: string, updates: Partial<IApplication>): Promise<IApplication | null> {
        ...
        if (updates.result) updatePayload.result = updates.result;
    ...
    private mapToApplication(row: any): IApplication {
        return {
